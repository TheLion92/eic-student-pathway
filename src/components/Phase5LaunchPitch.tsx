...
import { Phase5LaunchPitch } from "@/lib/phases/phase5-launch-pitch";
import { ChevronDown, ChevronRight, CheckCircle, ExternalLink, Download, Target, Rocket, DollarSign, Presentation, Users, Trophy } from "lucide-react";
...
interface Phase5LaunchPitchProps {
  onComplete: () => void;
}

const Phase5LaunchPitchView = ({ onComplete }: Phase5LaunchPitchProps) => {
  const [expandedTask, setExpandedTask] = useState<string | null>(null);
  const [taskStatuses, setTaskStatuses] = useState<Record<string, TaskStatus>>({});

  const completedTasks = Object.values(taskStatuses).filter(status => status.completed).length;
  const totalXP = Phase5LaunchPitch.tasks.reduce((sum, task) => sum + task.xp, 0);
  const earnedXP = Phase5LaunchPitch.tasks.reduce((sum, task) => 
    sum + (taskStatuses[task.id]?.completed ? task.xp : 0), 0
  );
  const progress = (completedTasks / Phase5LaunchPitch.tasks.length) * 100;
  ...
  const markTaskComplete = (taskId: string) => {
    ...
    const task = Phase5LaunchPitch.tasks.find(t => t.id === taskId);
  ...
  const getTaskIcon = (task: any) => {
    switch (task.type) {
      case "video": return <Trophy className="w-4 h-4" />;
      ...
      case "research": return <Rocket className="w-4 h-4" />;
      case "experiment": return <Rocket className="w-4 h-4" />;
      case "submission": return <Trophy className="w-4 h-4" />;
  ...
  const isPhaseReady = Phase5LaunchPitch.tasks.every((t) => taskStatuses[t.id]?.completed);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-2 border-orange-200 bg-gradient-to-r from-orange-50 to-red-50">
        <CardHeader className="text-center">
          <div className="flex items-center justify-center gap-3 mb-4">
            <Badge className="bg-orange-600 text-white text-sm font-medium">
              Phase 5
            </Badge>
            <span className="text-3xl font-bold text-gray-900">{Phase5LaunchPitch.title}</span>
          </div>
          <p className="text-lg text-gray-700 mb-4">{Phase5LaunchPitch.goal}</p>
          <div className="bg-white p-4 rounded-lg border">
            <div className="prose prose-sm max-w-none text-left" 
                 dangerouslySetInnerHTML={{ __html: Phase5LaunchPitch.intro_md.replace(/\n/g, '<br/>') }} />
          </div>
        </CardHeader>
      </Card>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <span className="text-sm text-muted-foreground">
                {completedTasks} of {Phase5LaunchPitch.tasks.length} tasks completed
              </span>
      ...
      <div className="space-y-4">
        {Phase5LaunchPitch.tasks.map((task) => {
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-orange-600 font-semibold">{task.xp} XP</span>
                    </div>
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {task.resources.map((resource, index) => (
                            <a
                              ...
                              className="inline-flex items-center gap-2 px-3 py-1 bg-orange-100 text-orange-800 rounded-md hover:bg-orange-200 transition-colors"
                            >
        ...
        <Card className="border-green-200 bg-green-50">
          <CardContent className="p-6 text-center">
            <h3 className="text-xl font-bold text-green-800 mb-2">
              🎉 EIC Pathway Complete!
            </h3>
            <p className="text-green-700 mb-4">
              Congratulations! You've successfully completed all 5 phases and earned {earnedXP} XP! 
              Visit the EIC to get your completion code and next-steps referral.
            </p>
...
export default Phase5LaunchPitchView;
