...
import { Phase3Build } from "@/lib/phases/phase3-build";
import { ChevronDown, ChevronRight, CheckCircle, ExternalLink, Download, Code, TestTube, Users, FileText } from "lucide-react";
...
interface Phase3BuildProps {
  onComplete: () => void;
}

const Phase3BuildView = ({ onComplete }: Phase3BuildProps) => {
  const [expandedTask, setExpandedTask] = useState<string | null>(null);
  const [taskStatuses, setTaskStatuses] = useState<Record<string, TaskStatus>>({});

  const completedTasks = Object.values(taskStatuses).filter(status => status.completed).length;
  const totalXP = Phase3Build.tasks.reduce((sum, task) => sum + task.xp, 0);
  const earnedXP = Phase3Build.tasks.reduce((sum, task) => 
    sum + (taskStatuses[task.id]?.completed ? task.xp : 0), 0
  );
  const progress = (completedTasks / Phase3Build.tasks.length) * 100;
  ...
  const markTaskComplete = (taskId: string) => {
    ...
    const task = Phase3Build.tasks.find(t => t.id === taskId);
  ...
  const isPhaseReady = Phase3Build.tasks.every((t) => taskStatuses[t.id]?.completed);
      ...
      <Card className="border-2 border-purple-200 bg-gradient-to-r from-purple-50 to-pink-50">
        <CardHeader className="text-center">
          <div className="flex items-center justify-center gap-3 mb-4">
            <Badge className="bg-purple-600 text-white text-sm font-medium">
              Phase 3
            </Badge>
            <span className="text-3xl font-bold text-gray-900">{Phase3Build.title}</span>
          </div>
          <p className="text-lg text-gray-700 mb-4">{Phase3Build.goal}</p>
          <div className="bg-white p-4 rounded-lg border">
            <div className="prose prose-sm max-w-none text-left" 
                 dangerouslySetInnerHTML={{ __html: Phase3Build.intro_md.replace(/\n/g, '<br/>') }} />
          </div>
        </CardHeader>
      </Card>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <span className="text-sm text-muted-foreground">
                {completedTasks} of {Phase3Build.tasks.length} tasks completed
              </span>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-purple-600 font-semibold">{task.xp} XP</span>
                    </div>
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {task.resources.map((resource, index) => (
                            <a
                              ...
                              className="inline-flex items-center gap-2 px-3 py-1 bg-purple-100 text-purple-800 rounded-md hover:bg-purple-200 transition-colors"
                            >
        ...
        <Card className="border-green-200 bg-green-50">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-green-700 mb-4">
              You've successfully completed all build tasks and earned {earnedXP} XP! 
              Visit the EIC to get your unlock code for Phase 4.
            </p>
...
export default Phase3BuildView;
